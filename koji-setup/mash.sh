...
if [[ "$MASH_BUILD_NUM" -ne "$KOJI_BUILD_NUM" ]]; then
	...
	rm -f "$COMPS_FILE"
	repoquery -q -a --repofrompath=mash,file://"$MASH_DIR_NEW"/clear/x86_64/os --queryformat="%{NAME}\t%{VERSION}\t%{RELEASE}" | sort > "$MASH_DIR_NEW"/clear/x86_64/packages-os &>/dev/null
	repoquery -q -a --repofrompath=mash,file://"$MASH_DIR_NEW"/clear/x86_64/debug --queryformat="%{NAME}\t%{VERSION}\t%{RELEASE}" | sort > "$MASH_DIR_NEW"/clear/x86_64/packages-debug &>/dev/null
	repoquery -q -a --repofrompath=mash,file://"$MASH_DIR_NEW"/clear/source/SRPMS --queryformat="%{NAME}\t%{VERSION}\t%{RELEASE}" | sort > "$MASH_DIR_NEW"/clear/source/packages-SRPMS &>/dev/null
	if [[ -e "$MASH_DIR" ]]; then
